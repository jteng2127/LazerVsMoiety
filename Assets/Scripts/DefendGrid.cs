...
public class DefendGrid : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        tileWidth = totalBox.size.x / old_GameManager.tileColumnTotal;
        tileHeight = totalBox.size.y / old_GameManager.tileRowTotal;
    }
    ...
    public GameObject create_tile(int row, int col){
        float dx = tileWidth*(col-old_GameManager.tileColumnTotal/2);
        float dy = tileHeight*(row-old_GameManager.tileRowTotal/2);
